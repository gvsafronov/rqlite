...
▗▄▄▄▖▗▖ ▗▖▗▄▄▄▖▗▄▄▖ ▗▄▄▄▖▗▄▄▄▖▗▖  ▗▖
▐▌   ▐▌ ▐▌  █  ▐▌ ▐▌  █    █   ▝▚▞▘ 
▐▛▀▀▘▐▌ ▐▌  █  ▐▛▀▚▖  █    █    ▐▌   The Streaming dbms for HTAP analytics
▐▌   ▝▚▄▞▘  █  ▐▌ ▐▌▗▄█▄▖▗▄█▄▖▗▞▘▝▚▖
 `         

const name = `Futriix`
const desc = `Futriix is a lightweight, distributed streaming database, which uses SQLite as its
storage engine. It provides an easy-to-use, fault-tolerant store for  data.
`
...
func main() {
	...
	log.Printf("%s starting, version %s, commit %s, branch %s, compiler (toolchain) %s, compiler (command) %s",
		name, cmd.Version, cmd.Commit, cmd.Branch, runtime.Compiler, cmd.CompilerCommand)
	...
	// Now, open store. How long this takes does depend on how much data is being stored by futriix.
	...
	stopProfile()
	log.Println("futriix server stopped")
}
